...
**Repository**: https://github.com/xiaoxipanda/stock-analysis-system
...
- 如果修复涉及用户可见行为、配置语义、CLI/API、部署、通知、报告结构、交易或风控，要同步更新相关文档和配置模板
- `README.md` 只承载项目定位、核心能力、快速开始、主要入口等首页级信息；非必要不更新 README
...
- 后端最低要求：`python -m py_compile <changed_python_files>`
- 前端：`cd apps/web && npm ci && npm run lint && npm run build`
